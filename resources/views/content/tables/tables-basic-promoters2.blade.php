...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light"></span>  Total App Promoters Registrations  ({{($promoters->count())}})
</h4>
...
<div class="card">
  <h5 class="card-header">App Registrations</h5>
    ...
    <table class="table" id="example"> 
      ...
      <tbody class="table-border-bottom-0">
        ...
        <tr>
          ...
          <td>#{{$promoter->id}}</td>
          <td><a href="#">{{$promoter->name}}</a> </td>
          <td>{{$promoter->email}}</td>
          <td>{{$promoter->phone}}</td>
          <td><span class="@if($promoter->status == "Activated") text-success @else text-danger @endif ">{{$promoter->status}}</span></td>
          <td>
            <div class="dropdown">
              <button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button>
              <div class="dropdown-menu">
                <a class="dropdown-item" href="/promoter/app/registration/activate/{{$promoter->id}}"><i class="bx bx-show me-1"></i> @if($promoter->status == "Not Active") Activate @else De-Activate @endif</a>
              </div>
            </div>
          </td>
        </tr>
